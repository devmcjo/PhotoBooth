...
;   {app}\branding.ini 를 읽기만 한다. **it27: {app}\Frame 은 더 이상 읽지 않는다**(번들 개념 폐기).
...
;      Frame\       … it26 이전 버전이 이 폴더에 남긴 프레임 캐시
;    사용자 요구가 "Program Files·로그·레지스트리 모두 제거"이므로 명시 열거한다.
Type: files; Name: "{app}\MCPhoto.ini"
Type: files; Name: "{app}\branding.ini"
; 구 프레임 캐시(재취득 가능 — 서버에서 다시 내려받는다). **it27 이후 앱은 이 폴더를 읽지도 않는다**
;   (고아 캐시) → 제거가 정리하는 것이 유일한 처분이다. 이 제품은 아직 배포된 적이 없으므로 실제 대상은
;   인스톨러를 돌려 본 개발·검증 머신이고, 지시행은 **회귀 방어**로 남긴다.
...
;    ⚠️ it27 근거 재규정: 이 제품은 아직 배포된 적이 없어 **현장 손님 사진은 존재하지 않는다**. 그래도
;       규약은 유지한다 — 근거가 "현장 자산 보호"에서 ① **회귀 방어**(훗날 저장 경로를 {app} 쪽으로
;       되돌릴 때 제거가 손님 사진을 지우는 사고가 되살아난다) ② 인스톨러를 돌려 본 **개발·검증 머신
;       보호**로 바뀐 것이다. 프레임 캐시(재취득 가능)와 사진(유일 사본 가능)의 위험 등급은 다르다.
