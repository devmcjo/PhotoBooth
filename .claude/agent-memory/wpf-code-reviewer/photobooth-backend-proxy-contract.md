...
**안전 불변식:** ⚠️ **it15(2026-07-28)에서 `AppSettings.UseBackend` feature flag와 레거시 `MCPhoto.Firebase` 직결 경로가 전면 폐지**됐다 — DI 분기는 사라지고 `ServiceRegistration.RegisterBackendServices`가 Http* 구현만 무조건 등록한다. `NormalizeBackend()`는 트림·트레일링 슬래시 보정만 하고 빈 base URL이면 아무 것도 되돌리지 않는다. 미구성은 `HttpFirebaseClient(configured:false)` → `IsInitialized=false` → 런타임 호출 실패로 드러난다. **BaseAddress에 `/api/` 경로가 있으므로 상대 URL은 반드시 선행 슬래시 없이** 시작해야 경로 보존(Clamp의 트레일링 슬래시 강제와 짝). `IsInitialized`=base URL 설정 사실(실시간 헬스체크 게이팅 금지 — 일시 지연에 업로드 오차단 방지, 도달성은 호출 실패→상위 폴백).
