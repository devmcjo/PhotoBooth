...
- **예외 — 키오스크 웹 클라이언트(`webclient/`)는 이터레이션이 아니라 WBS Step으로 진행**한다(`docs/web-client/11-wbs.md`). 그 단위의 설계 문서는 `web-step{N}-{topic}-design.md`를 쓴다(예: `web-step9-timelapse-encoder-design.md`). `web-it{N}-*`는 **다운로드 페이지(`web/`)** 쪽 문서다 — 두 "web"은 서로 다른 산출물이다.
  - ⚠️ 접미사가 일관되지 않다: Step 9·10·11은 `-design`이 있고 **Step 12~15는 없다**(`web-step12-google-sso-auth.md`·`web-step13-settings-pin-gate.md`·`web-step14-frame-catalog-and-select.md`·`web-step15-frame-editor-and-picker.md`). 파일을 찾을 때 `web-step*` 로 glob한다.
