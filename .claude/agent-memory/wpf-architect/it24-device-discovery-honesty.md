...
- **프린터 (b) 범위 — ⚠️ it25에서 폐기됨**: it24의 "설치 프린터 열거+선택+저장"은 사용자 반려("지원 항목 0 — 설치 프린터 나열 자체가 오표시") → it25가 placeholder로 환원, `PhotoPrinterName` 키·`IPrinterEnumerator` 삭제. 교훈: **"설치된 것 나열"이 아니라 "지원+인식된 것 나열"이 이 사용자의 장치 노출 기준**이다(카메라 §4.3 동형 원칙). 프린터 재개 조건 = 지원 모델 ≥1.
