...
- 게스트(비로그인)는 촬영·합성·필터·로컬 저장을 모두 쓸 수 있다. 로그인이 필요한 것은 **QR 전송(업로드)**·커스텀 프레임·계정 관리·관리 기능이다.
  - ⚠️ **게스트에게는 QR이 제공되지 않는다**(2026-07-30 소스 대조 정정): `QrEffectivePolicy.IsQrEnabled`(`src/MCPhoto.Core/Settings/QrEffectivePolicy.cs`, it13 §7.1b)가 미로그인이면 `false`를 돌려 `Result`에서 **즉시 세션 완료**(홈 복귀 + 완료 토스트)로 끝난다. 서버 업로드 게이트(`optionalBearer`)는 무토큰을 허용하지만 **클라이언트가 업로드 자체를 시작하지 않는다** — QR은 계정 단위 과금·한도(TempUser)의 대상이기 때문이다. 종전의 "게스트가 업로드·QR을 쓸 수 있다"는 서술은 **서버 게이트와 클라이언트 정책을 혼동한 오기**였다.
...
| 서버 SSO 미구성 **또는 OAuth 클라이언트 자격 오류**(Google이 `invalid_client`/`unauthorized_client`로 code 교환 거부) | **501** 응답 | "Google 로그인이 구성되지 않았습니다. 관리자에게 문의하세요." |
| 백엔드 미도달 | 네트워크 실패 | "Google 로그인 중 오류가 발생했습니다. 네트워크를 확인해 주세요." (화면 유지) |

- **OAuth 클라이언트 자격 오류가 501인 이유**: Google 토큰 엔드포인트의 `invalid_client`/`unauthorized_client`는 우리 서버의 client_id/secret이 틀렸다는 뜻뿐이고 **계정 존재 여부와 무관**하다. 따라서 401 일반화(계정 열거 방지)의 대상이 아니며, 401로 감추면 운영자가 구성 오류를 계정 문제로 오인한다(2026-08-01 실제 발생). ⚠️ `invalid_grant`는 만료·재사용된 code에서도 나오므로 **여기 포함하지 않는다**(손님 흐름 문제 → 401 유지).
...
| 프레임 목록(공용) | 로컬 캐시·fallback으로 폴백([13 §5](./13-client-behavior-spec.md)) |
