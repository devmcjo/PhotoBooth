...
입력·출력 모두 3채널 8bit. **원본을 변형하지 않고 새 버퍼를 반환**한다. 컷 **전체 일괄** 적용이며 얼굴 인식·형상 변경(워프)은 없다. 단 뷰티는 **피부톤 색역 마스크**로 적용 영역을 가린다(얼굴 검출이 아니라 색 기준이다).
...
| **뷰티**(Beauty) | ① 피부톤 마스크 → ② 엣지 보존 스무딩 → ③ 마스크 가중 블렌드 → ④ 감마 톤업 → ⑤ 채도 → ⑥ 언샵 | 아래 §6.1 |

- 결과값은 **채널당 0~255로 포화(saturate)** 처리한다(랩어라운드 금지).
- 설정 토글은 **노출 여부**만 제어한다. 실제 적용은 결과 화면에서 사용자가 고른다. "원본"은 항상 목록에 있고 끌 수 없다.

### 6.1 뷰티 규격 (2026-08-07 개정)

**개정 이유**: 종전 규격(전면 bilateral `d=7` 고정 + `smooth*0.6 + src*0.4` + `alpha=1.03/beta=6`)은 **고해상도에서 효과가 보이지 않았다.** 커널이 절대 픽셀 고정이라 1080p 이상에서는 상대적으로 너무 작고, 전면 적용이라 강도를 올리면 눈·머리카락까지 뭉개져 올릴 수도 없었다. 실사용 평가는 "흑백·밝게는 알겠는데 뷰티는 구분이 안 된다"였다.

| 단계 | 규격 | 파라미터 |
|------|------|----------|
| ① 피부톤 마스크 | YCrCb 색역 임계 → 모폴로지 **열기→닫기** → 가우시안 블러로 **소프트 에지** → 0~1 실수 마스크 | `Cr ∈ [133,173]`, `Cb ∈ [77,127]` · 모폴로지 커널 = **타원, `max(3, min(W,H)/160)` 홀수** · 블러 커널 = **커널×3 홀수** |
| ② 엣지 보존 스무딩 | **절반 해상도로 축소(area) → bilateral 2회 → 원본 크기로 확대(linear)** | 1회 `d=9, sigmaColor=60, sigmaSpace=9` · 2회 `d=9, sigmaColor=45, sigmaSpace=9` |
| ③ 마스크 블렌드 | `dst = src*(1-w) + smooth*w`, `w = mask * strength` (픽셀별) | **strength = 0.85** |
| ④ 감마 톤업 | LUT: `out = (in/255)^γ * 255` | **γ = 0.88** |
| ⑤ 채도 | HSV의 **S만** 배율(포화 연산) | **1.08** |
| ⑥ 언샵 | `dst = src*(1+a) - blur*a` | **a = 0.22**, 블러 커널 = `max(3, min(W,H)/200)` 홀수 |

**규격의 핵심 3가지**(플랫폼 대체 시 반드시 유지)

1. **커널은 해상도 비례여야 한다.** 절대 픽셀 고정은 고해상도에서 무력해진다. 절반 해상도 처리는 큰 커널과 시각적으로 같고 비용은 1/4이다(합성은 슬롯 수만큼 반복된다).
2. **스무딩은 피부 영역에만.** 전면 적용은 강도를 못 올리는 구조적 한계가 있다. 마스크가 있으면 눈·머리카락·배경 디테일을 지키면서 피부만 세게 다룰 수 있다.
3. **톤업·채도·언샵이 함께 있어야 "보정"으로 읽힌다.** 스무딩만으로는 사용자가 차이를 인지하지 못한다.

**플랫폼 대체**: bilateral이 없으면 **가이디드 필터 / 표면 블러 / 엣지 보존 블러**로 근사한다. WebGL 등에서 정확한 동일 픽셀이 불가하면 위 3가지 성질을 유지하는 것이 규격이다. 골든 이미지 비교(`docs/spec-vectors/golden/expected-beauty.png`)는 **허용 오차 내 일치**를 요구한다.

> ⚠️ **형상 변경 금지**: 얼굴 슬리밍 같은 워프는 이 규격에 없다. 오검출 시 얼굴이 왜곡된 사진이 그대로 손님에게 전달되는 되돌릴 수 없는 실패이기 때문이다. 검토 결과·구현 경로는 [90 로드맵](./90-roadmap-and-future-work.md).
