...
| **QR 전송·업로드(effective QR)** | **×** | △(한도 내) | ○ | ○ | ○ | ○ | `QrEffectivePolicy.IsQrEnabled`(it13 §7.1b) — **미로그인 → false(Result에서 즉시 세션 완료 = 홈 복귀 + 완료 토스트)**, TempUser 한도 초과 → false, 그 외 raw 설정 그대로. 호출부 `ResultViewModel.Next`. 서버 업로드 게이트(`optionalBearer`)는 무토큰을 허용하지만 클라이언트가 시작하지 않는다(2026-07-30 행 분리 정정) |
