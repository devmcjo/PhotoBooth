...
> ⚠️ **이 절의 판정은 2026-07-30 사용자 결정으로 대체됐다.** 웹 클라이언트는 **Windows 앱의 전 화면(P1+P2+P3+P4)을 구현**하는 것으로 범위가 확정됐다. 위 §7.4.1의 W-BLOCK 5건에 대한 재판정과 해결 방식(타임랩스 = 브라우저 직접 인코딩, 결과물 보관 = OPFS 3계층/M6-W, 개인 프레임 = 웹이 스스로 소비자가 되어 근거 소멸, 로그 = IndexedDB 링버퍼)은 **[`docs/web-client/00-scope-and-decisions.md §3`](../web-client/00-scope-and-decisions.md)** 에 있다. 아래 표는 **이력**으로 남긴다. §7.4.1~§7.4.4의 브라우저 API 제약 서술 자체는 사실이며 계속 유효하다.
