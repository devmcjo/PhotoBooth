...
- 반영된 설계 결정 요약은 `docs/design/wpf-architecture.md` 및 `docs/design/wpf-it2~it9-design.md`에 근거(롱프레스 폐지·게스트 직행·계정 단일 소스·에디터 좌표 순수함수화·QR 세분화·유휴 경고 팝업 등). 본 문서는 실제 소스 기반으로 작성했으며, 세부 동작은 위 인용 경로가 진실의 소스다.
