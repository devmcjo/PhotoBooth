...
- **항목**(2열 그리드 + 그룹, `SettingsView.xaml`):
  ...
  - **외부 장치(it23 신설 → it24 개편)**: **항상 표시 + 읽기 전용**이다. it23까지는 섹션 전체가 게스트에게 Collapsed였는데, 그것이 "외부 장치 연결 탭이 사라졌다"는 사용자 혼란의 직접 원인이었다(같은 화면의 다른 로그인 전용 항목은 전부 "보이되 Disable + 로그인 필요 노티"이며 섹션째 숨김은 이 섹션만의 예외였다). it24에서 폐지 — 게이트는 편집 컨트롤에만 걸린다.
    - 외부 카메라 사용 토글 → on일 때 **지원 모델** ComboBox(레지스트리 값 기반) + 노출 3요소(**슬라이더 + 직접 입력 병행**) + **[장치 검색]**.
    - 캡션 4종: "타임랩스 기능은 웹캠으로만 동작됩니다."(토글 on일 때), **"이 앱이 SDK 연동을 지원하는 모델 목록입니다. 연결된 장치 목록이 아닙니다"**(it24 — 지원 모델과 연결된 장치를 섞지 않게 하는 구분), 화각 불일치 고지, "노출 목록은 카메라 연결 시 확인됩니다."(도메인 미확보 시).
  ...
  - **프린터(it24 — placeholder 해제)**: 프린터 사용 토글이 편집 가능해졌고(게이트는 외부 카메라와 동일), on일 때 하위 패널에 **설치 프린터 ComboBox + [다시 검색]**이 나온다. 열거는 `System.Printing`(`LocalPrintServer.GetPrintQueues`, Local+Connections)이며 관리자 권한을 요구하지 않는다.
    - **실제 인쇄는 아직 없다** — 하위 패널에 상시 고지("인쇄 기능은 아직 제공되지 않습니다. 선택한 프린터는 인쇄 기능이 추가되면 사용됩니다."). 이 고지가 있는 한 "미리 골라 두기"는 거짓이 아니라 준비다.
    - 열거 시점: 토글 on 상태로 설정 진입 시 자동 1회 + off→on 전환 시 1회 + [다시 검색]. off면 열거하지 않는다. 스풀러 DB 조회는 USB 세션을 만들지 않아 "진입만으로 USB를 건드리지 않는다"는 결정과 충돌하지 않는다.
    - **상태 구분**: 성공·0대 = "설치된 프린터가 없습니다"(스풀러 DB 기준 명제) / 열거 실패(스풀러 중지 등) = **"프린터 목록을 확인할 수 없습니다 (인쇄 스풀러 상태 확인)"**. 조치가 다르므로 뭉개지 않는다.
    - **오프라인 여부는 표시하지 않는다** — 스풀러의 오프라인 플래그는 stale해지기 쉽고, 틀린 "오프라인" 딱지는 화면이 하는 거짓말이다. "설치됨"까지만 말한다.
    - **저장값은 목록 부재를 이유로 지우지 않는다**: 저장된 이름이 목록에 없으면 "{이름} (설치 확인 필요)" 합성 행을 첫 항목으로 넣어 선택을 유지한다(프린터가 잠시 꺼진 것일 수 있고, 항목이 없으면 WPF 콤보가 `SelectedValue`를 null로 되써서 관리자 값이 저장 한 번에 사라진다).
