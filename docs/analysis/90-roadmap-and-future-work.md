...
| ~~ffmpeg GPL 준수 미이행~~ | GPLv3 바이너리(`--enable-gpl --enable-version3`)를 재배포하면서 라이선스 전문·고지·대응 소스 안내를 아무것도 동봉하지 않았다 → **위반 상태**. 별도 프로세스 구조라 MCPhoto 소스 공개 의무는 없었으나, 재배포자 의무(GPLv3 §4·§6)는 별개다 | `licenses/`, `MCPhoto.App.csproj`, `Views/DiagnosticsWindow.xaml` | **이행 완료(2026-08-06)**: `licenses/`에 GPLv3 전문·configuration 전문·소스 URL·**3년 서면 오퍼** 동봉 + 빌드/publish 복사 배선 + 진단 창 "오픈소스 라이선스" 카드(누락 시 경고). 회귀 방지 `LicenseComplianceTests` 8건. 잔여는 **소스 미러 호스팅(U-1)** — 현재는 제3자 링크 + 서면 오퍼로 충족. 상세: [ffmpeg 라이선스·배포 설계 §10](../design/wpf-ffmpeg-licensing-and-distribution-design.md) |
...
- **얼굴 인식 기반 뷰티(얼굴 슬리밍 등 형상 보정)** — 2026-08-07 사용자 요청, **가능성 검토 후 이번 범위 제외**. 대신 색 기준 피부톤 마스크로 뷰티 강도를 올렸다([14 §6.1](./14-media-pipeline-spec.md)).
  - **검토 결과(Windows/OpenCvSharp 4.10 기준)**: 얼굴 **위치 검출**은 가능하다 — `CascadeClassifier`·`DetectMultiScale` API가 바인딩에 존재하고, Haar/LBP 캐스케이드 XML(약 500KB, OpenCV BSD-3)을 앱에 동봉하면 된다. `Facemark`(랜드마크) API도 존재한다. 단 **패키지에 모델 데이터가 없다**(`opencvsharp4.windows` nupkg 확인) → 전부 직접 동봉해야 한다. `FaceDetectorYN`(YuNet DNN)은 이 바인딩에 **없다**.
  - **막히는 지점은 검출이 아니라 워프다**: "갸름하게"는 검출 사각형만으로 부족하고 **턱선 랜드마크**가 필요하다. `Facemark` LBF 모델(`lbfmodel.yaml`)은 **약 54MB**로, 배포물이 **self-contained 단일 exe**라는 제약([80 빌드·배포](./80-build-and-deployment.md))에 정면으로 부딪힌다.
  - **더 중요한 위험**: 워프는 **되돌릴 수 없는 실패 모드**를 만든다. 오검출·부분 가림·다인 촬영에서 얼굴이 일그러진 사진이 그대로 손님에게 전달되고, 키오스크에는 "이 보정 취소" 동선이 없다. 필터는 언제나 **안전한 방향으로만 실패**해야 한다.
  - **착수 조건**(전부 충족 시): ① 모델 배포 방식 결정(동봉 vs 최초 실행 시 다운로드 — 오프라인 요건과 충돌 검토), ② 검출 실패·다인 촬영 시 **형상 보정 자동 미적용** 폴백, ③ 결과 화면에서 **보정 강도 0 포함 조절** 제공, ④ 성능 예산(슬롯 수 × 원본 해상도 워프)이 필터 전환 체감을 해치지 않음, ⑤ 웹 클라이언트 대체 경로(MediaPipe FaceMesh 등) 확인 — [12 웹 차이](../web-client/12-web-vs-windows-differences.md)에 등재해야 한다.
  - **중간 단계 대안**(리스크 낮음): 얼굴 **검출까지만** 도입해 스무딩 강도를 얼굴 영역에서 높이는 것. 형상은 건드리지 않으므로 위 위험이 없고 캐스케이드 XML 500KB만 필요하다. 다만 현재 피부톤 마스크 대비 체감 이득이 크지 않아 슬리밍과 함께 판단하는 것이 낫다.
