...
### 2.4 오버레이 복귀 (`analysis/13 §2.3`)

- `Settings`·`Login`·`Account`는 **진입 전 상태를 복귀 지점으로 보존**한다.
- 진입 시 현재 상태가 `Settings`/`Login`이 **아닐 때만** 복귀 지점을 갱신한다(중첩 진입 시 원래 화면을 잃지 않게).
...
| 버전 캡션 | 하단에 `v{version} · {site}` 흐린 캡션, **로그인 무관 상시**, 클릭 비간섭 |
...
| 검증 | E2E 테스트로 **"로그아웃 직후 게스트 업로드 요청에 `Authorization` 헤더가 없음"** 을 고정한다([10 §5](./10-testing-and-acceptance.md)) |
