...
### 4.6 ~~내보내기 / 가져오기 번들~~ → **2026-08-07 폐기**

> 🚫 **이 절의 규격은 더 이상 구현 대상이 아니다.** 프레임 zip 내보내기/가져오기 기능을 **제거**한다.
>
> **폐기 사유**: 이 기능의 명분은 `WB3`("브라우저 저장소가 회수돼도 내보내기로 복구 가능")이었다.
> 개인 프레임이 **서버 정본**이 되면서([WD4 개정](./00-scope-and-decisions.md)) 복구가 재로그인
> 한 번으로 이뤄진다 — 명분이 소멸했다.
>
> 게다가 아래 규격은 이미 **Windows와 어긋나 있다**: `.slots`가 v2(서명 포맷)로 바뀌었고
> 파일명 접두 규칙도 폐지됐다. 웹은 `.slots`를 다루지 않기로 확정됐으므로 맞출 수도 없다.
>
> 삭제 범위: [17 §4.4](./17-windows-changes-2026-08-07.md#44-프레임-zip-내보내기가져오기--기능-폐기-)
> · 프레임 공유는 **서버 경유가 유일한 경로**다.
...
| 파일명 규칙 | Windows 앱의 `Frame\` 폴더에 그대로 풀어 넣으면 인식되는 형태(접두 규칙 준수) |
| 가져오기 | zip 해석 → 이름·슬롯 검증 → 10개 상한 확인 → 충돌 시 사본 이름 규칙 적용 |
| zip 구현 | 순수 JS(압축 없이 `store` 방식) |
...
| 프레임 | **서버 정본**이므로 같은 계정으로 로그인하면 그대로 보인다. ~~zip 번들~~은 폐기됐다(§4.6) |
