...
| 작성일 | 2026-07-30 |
| 상태 | **구현 완료 — WBS Step 0~17 전부 + 서버 선행작업(B1·B2·B4) + 사용자 액션 A1~A5 완료**(2026-08-01, 브랜치 `feature/web-client-foundation`, 웹 vitest **1926**(84파일) / **E2E 44**(Playwright chromium 29 + webkit 15) / 서버 316 / Windows 938 통과). **남은 것은 실기기 실측 84항목** → [16 · 실기기 검증 통합 절차서](./16-field-verification-runbook.md). 관례·재개는 [15](./15-implementation-conventions.md), 사람이 할 일은 [14](./14-handoff-and-user-actions.md) |
| v1.4 갱신(2026-07-31) | **`main` 머지분 반영**(`e5efdfd`) — ① **it20 프레임 로딩 4국면**(`Loading`/`Ready`/`Degraded`/`Failed`, 무진행 30초·총 60초 상한, 진행 보고 `(n/m)`, 단일 비행 로더) 신설 → `03 §4.1`·`06 §6.1`·`02 §6.2`·`11 Step 14` ② **기존 프레임 불러오기 = 사본이 아니라 신규 생성으로 재정의** + **서버 등록 확인 모달**(기본 on·원자성) 신설 + 저장 전 검증 7단 + `isFileNameSafe` 분리 → `03 §11.3~§11.5`·`11 Step 15`(**모달 6종 → 7종**) ③ 선행 정정: 두 변경이 `docs/analysis`에 미반영이었으므로 **규격(11 §3·§4.1, 13 §4.2·§6.3~§6.5)을 먼저 고쳤다** |
...
- 만드는 것: **브라우저 전체화면에서 도는 셀프 포토부스 앱**. Windows 앱(`MCPhoto.App`)의 13개 화면 + 팝업 9종 전부(**셸 모달 4** + **화면 로컬 오버레이 5** — [00 §2](./00-scope-and-decisions.md)).
...
| 6 | [03 · 화면별 상세 명세](./03-screens-spec.md) | 13화면 + 팝업 9종의 레이아웃·상태·명령·문구·검증 | 구현 시 상시 참조 |
...
| 16 | [15 · 구현 관례와 재개 가이드](./15-implementation-conventions.md) | **이 저장소에서는 어떻게 짜는가** — 계층·테스트 전략·이미 밟은 함정 10건·Step 9~16 사전 정보 | **코드 작성 전 필독** |
| 17 | [16 · 실기기 검증 통합 절차서](./16-field-verification-runbook.md) | **V1~V25 실측 84항목**을 기기 3대 × 계정 5종의 세션 S1~S9로 재편한 수행 절차 + 진행 기록란 | **구현 완료 후 · 출시 전** |
| 18 | [17 · 2026-08-07 Windows 변경 인수인계](./17-windows-changes-2026-08-07.md) | 그날 Windows·서버에 일어난 **변경분만** 모은 웹 작업 지시서 — 지금 깨진 것 / 서버 API 변경 / C11~C16 착수 순서 / 미결정 3건 / 함정 8건 | **⚠️ 지금 최우선** |

> 🔴 **2026-08-07 이후 이어받는다면 [17](./17-windows-changes-2026-08-07.md)을 먼저 읽는다.** Windows·서버가 그날 크게 바뀌었고,
> 공유 벡터·골든 파일이 Windows 기준으로 갱신돼 **웹 테스트 일부가 지금 실패 상태**다. 미결정 3건은 착수 전에 답을 받아야 한다.

> ⚠️ **구현을 이어받는다면 [15](./15-implementation-conventions.md)를 먼저 읽는다.** 00~13은 "무엇을 만드는가"이고, 15는 "이 저장소에서 어떻게 만들어 왔는가"다. 15를 건너뛰면 기존 코드와 어긋난 방식으로 짜게 된다.
>
> ⚠️ **[16](./16-field-verification-runbook.md)은 순서·기록만 소유한다.** 실측 항목의 *정의*는 [14 §10](./14-handoff-and-user-actions.md)이 진실원이다 — 두 문서가 어긋나면 14가 사실이다.
