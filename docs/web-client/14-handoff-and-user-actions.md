...
| 상태 | **WBS Step 0~11 코드 완료(★마일스톤 A) + 사용자 액션 A1~A5 전부 완료·검증(2026-07-31).** 남은 것은 Step 12~17 개발과 §10의 실측뿐이다 |
...
- **Step 12(로그인) 개발은 A1~A3이 끝나 있어 지금 시작할 수 있다.** Step 11(업로드·QR)까지 완료돼 **게스트 완주 경로가 코드상 완성**됐고, 남은 실측은 §10.1~§10.5다.
...
### 10.5 V20 · Step 11(업로드 3단계 · QR · 완료) 실기기 실측 — **일부만 지금 가능**
...
V20-1~V20-3은 **effective QR을 목으로 `true` 고정**하거나 세션 사용자를 주입해 `Qr`까지 가야 한다.
...
| V20-4 | **폰으로 QR을 스캔해 다운로드 페이지가 열린다**(A4-가정) | 기본 카메라 앱이 인식 → P1 페이지 → 사진·영상 다운로드. **kiosk 도메인이 아니라 P1 도메인**이 열린다 | 물리 폰과 카메라가 필요하다 | **Step 12(로그인)** |
...
| 항목 | 필요 선행 |
|------|-----------|
| 폰으로 QR 스캔 → 다운로드(**V20-4**) | A1~A5 + Step 11(완료) + **Step 12(로그인)** |
| 10컷 세션 메모리(iOS 탭 생존) | Step 7 |

> **해소됨**: "업로드 `OPTIONS 204 → PUT 200`"은 Step 11 완료로 **지금 실측 가능**해졌다 → **V20-1**로 이동.
