...
- **Step 12(로그인)까지 코드가 완성됐다**(2026-08-01). 게스트 완주 경로(마일스톤 A) + Google SSO 로그인이 모두 구현돼 있고, 남은 실측은 §10.1~§10.6이다.
...
> **PIN 1회 오입력이 로그아웃을 유발하지 않는다(E17)** 는 화면(PIN 모달)이 없어 관측할 수 없다 →
> **Step 13 이후**로 이월한다(§10.7). 코드 쪽 보장은 `verifyMyPin`의 `unauthorized: "reject"` +
> `tests/unit/auth/sessionExpiry.test.ts`의 PIN 절이 이미 고정한다.

### 10.7 아직 불가능한 실측

| 항목 | 필요 선행 |
|------|-----------|
| PIN 1회 오입력이 로그아웃을 유발하지 않는다(E17 화면 관측) | **Step 13**(PIN 모달) |
| 10컷 세션 메모리(iOS 탭 생존) | Step 7 |

> **해소됨**: "업로드 `OPTIONS 204 → PUT 200`"은 Step 11 완료로 **V20-1**로 이동했고,
> "폰으로 QR 스캔 → 다운로드(V20-4)"는 Step 12 완료로 **지금 실측 가능**해졌다 → **V21-5**(§10.6)로 이동.
