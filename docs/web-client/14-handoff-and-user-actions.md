...
| 상태 | **WBS Step 1~7 + 서버 선행작업(B1·B2·B4) 코드 완료.** 아래 A1~A5를 처리하면 실기기·종단 검증이 열린다. 사람이 눈으로 봐야 하는 실측 항목은 §10 |
...
- **Step 8(합성·필터) 개발은 이 중 아무것도 기다리지 않는다.** Step 6·7(카메라·촬영)은 완료됐고, 실측(§10.1)은 `npm run dev`로 지금 가능하다.
...
```bash
cd webclient     && npx tsc --noEmit && npx vitest run     # 482 통과 (도메인 커버리지 100%)
...
**Step 8(합성 + 필터 + 골든 이미지)** 이 다음이며 **A1~A5를 기다리지 않는다.**

크리티컬 패스: `8 → 10 → 11`(★ 마일스톤 A = 게스트 촬영 완주). 이 경로는 **A4(CORS)·A5(배포)만** 필요하고 로그인 관련 A1~A3은 Step 12에서 필요해진다.
