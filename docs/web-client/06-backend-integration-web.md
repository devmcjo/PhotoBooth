...
| CORS 실패 시 | 그 프레임을 **목록에서 제외하지 않는다.** 썸네일은 원격 URL의 `<img>`로 보여주되(오염은 canvas에만 영향) **선택 자체를 막는다** — 카드에 `aria-disabled`와 캡션 *"이 프레임을 불러올 수 없습니다."* 를 붙인다. ⚠️ **정정(2026-08-01, Step 14)**: 종전 문구는 "선택 시 안내"였으나, 선택을 허용하면 손님이 **6컷을 다 찍은 뒤 `Result`에서야** 합성 실패를 만난다. `hasUsableImage`가 존재하는 이유가 그 실패를 촬영 **앞으로** 당기는 것이다. 로그에는 "프레임 이미지 다운로드 실패(네트워크 또는 CORS 차단 가능)"가 남는다 |
| 이미지 없는 문서 | 서버는 문서를 먼저 만들고 이미지 PUT은 나중이라 **이미지 없는 문서가 존재할 수 있다**(`analysis/31 §4.10`) → 로드 실패를 **크래시 없이** 처리한다 |

### 6.1 카탈로그 로딩의 진행 보고 (it20 — `03 §4.1`이 소비한다)

`FrameSelect`의 대기 오버레이는 **진행 문구 + (n/m) 카운터**를 보여주고, 무진행 30초 타이머를 **진행 보고마다 재무장**한다. 따라서 카탈로그 로더는 조회·다운로드 진행을 **밖으로 보고**해야 한다. 보고가 없으면 정상 진행 중인 다운로드가 30초에 잘려 거짓 실패가 뜬다.

| 단계 | 보고 시점 | 문구 |
|------|-----------|------|
| 로컬 확인 | 캐시 스캔 시작 | 설치된 프레임을 확인하는 중… |
| 서버 조회 | `GET /frames/default` 발신 | 서버에서 기본 프레임 목록을 확인하는 중… |
| 이미지 다운로드 | **각 이미지마다**(`index`/`total`) | 기본 프레임 내려받는 중… (n/m) |
| 완료 | 마지막 보고 | 프레임 목록을 정리하는 중… |

| 규칙 | 내용 |
|------|------|
| 문구 조립 | **도메인 순수 함수**가 담당한다(화면이 문자열을 조립하지 않는다) → UI 없이 단위 테스트된다 |
| 시작 문구 | 아직 어떤 보고도 없을 때: *"기본 프레임을 준비하고 있어요…"* |
| **단일 비행(single-flight)** | 동시 호출은 **하나의 작업을 공유**한다(중복 다운로드 0). 부트스트랩 prefetch가 이미 받는 중이면 화면 진입은 줄 서지 않고 **그 작업에 합류**해 최근 보고를 **즉시 replay** 받는다 |
| 취소의 범위 | **호출자별**이다. [기다리지 않고 시작]으로 화면이 대기를 접어도 **공유 작업은 계속 진행해 캐시를 완성한다** → 잠시 뒤 [다시 시도]가 성공한다 |
| 완료 보고 | 늦게 합류한 구독자를 위해 **완료도 하나의 보고**로 남긴다(replay 대상) |
