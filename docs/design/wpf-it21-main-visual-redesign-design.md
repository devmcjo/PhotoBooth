...
### 7.4 층4 — 안내 문구 (개정: 버튼 → 문구)

> **개정 이력**: 최초 설계는 여기에 **[로그인하고 내 프레임 쓰기] Ghost 버튼**을 두었다(§4.3 C7 보정).
> 실사용 확인 후 **버튼을 폐기하고 흐름 스트립 안의 조건부 문구로 대체**한다. 사유는 아래.

#### 폐기 사유

| # | 이유 |
|---|------|
| 1 | **주 액션이 둘로 보인다.** 어트랙트 화면의 원칙은 "주 액션 1개"(§0.4)인데, hero 아래 테두리 있는 버튼이 하나 더 있으면 손님이 무엇을 눌러야 할지 판단해야 한다 |
| 2 | **로그인은 손님 액션이 아니다.** 이 부스에서 로그인은 운영자·단골의 행위다. 처음 온 손님에게 로그인 버튼을 들이미는 것은 상용 키오스크의 관례가 아니다 |
| 3 | 정보를 **맥락이 있는 자리**에 두는 편이 낫다 — "프레임을 고른다"는 이야기는 이미 흐름 스트립 1번 칸이 하고 있다 |

C7(아이콘 단독은 모호하다)의 보정은 **상단 바 계정 아이콘의 ToolTip + Automation 이름**으로 유지되고, 아래 게스트 문구가 "로그인이라는 것이 있다"는 사실을 대신 전달한다.

#### ⚠️ 문구를 상태별로 나눠야 하는 이유 (정확성 문제)

프레임 **만들기**는 `CanWriteFrames` = **AdvancedUser · Manager · Admin** 뿐이다(`UserRole.cs:63-64`, it16 E2). 게스트는 물론 **일반 `user`·`temp_user`도 만들 수 없다.**

따라서 "직접 만들 수도 있어요"를 무조건 노출하면 **대다수 사용자에게 거짓**이 된다. 기대를 만들어 놓고 프레임 선택 화면에서 [프레임 만들기] 버튼을 찾지 못하게 하는 것은 상용 제품에서 가장 피해야 할 종류의 UX다. `IsPower`와 `CanWriteFrames`는 **별개 축**이라는 점도 주의한다(AdvancedUser는 `CanWriteFrames=true`, `IsPower=false`).

#### 확정 — 흐름 스트립 1번 칸의 조건부 보조 문구

| 상태 | 문구 | 참인가 |
|------|------|--------|
| `CanWriteFrames` (AdvancedUser+) | **"직접 만들 수도 있어요"** | ✅ 프레임 선택 화면에 [프레임 만들기]가 실제로 노출된다 |
| 게스트 | **"로그인하면 내 프레임을 쓸 수 있어요"** | ✅ 로그인 시 공용 + 자기 소유 프레임이 목록에 더해진다 |
| 로그인했지만 쓰기 권한 없음(`user`·`temp_user`) | **(없음)** | 이미 로그인했고 만들 수도 없다 — 할 말이 없으면 아무 말도 하지 않는다 |

- 위치: 1번 칸 설명("원하는 틀을 고르세요") **아래 한 줄**, `Brush.Accent.Text` 강조 + `SemiBold`, `Text.Caption` 크기.
- 칸 높이가 서로 달라지므로 흐름 스트립 셀은 **`VerticalAlignment="Top"`** 정렬을 유지한다.
- ⚠️ **흐름 스트립 칸 폭은 200 → 228**로 넓히고 보조 문구에 **`TextWrapping="Wrap"`을 명시**한다. `Text.Caption` 스타일에는 `TextWrapping`이 없어(`Text.Body`에만 있다) 200px에서 게스트 문구의 **양끝이 잘렸다**(렌더 검증에서 확인). 228 + Wrap이면 게스트·권한자 문구 모두 한 줄에 들어간다.
- Compact(<1008)에서는 흐름 스트립째 접히므로 이 문구도 함께 사라진다 — 창모드는 운영자 용도이고 손님 접점은 전체화면이라 수용한다.

#### `HomeViewModel` 확장

```csharp
/// <summary>게스트 여부(진입 시점 스냅샷). Home VM은 홈 진입마다 새로 생성되므로 통지 불필요(F12).</summary>
public bool IsGuest => _shell.IsGuest;

/// <summary>흐름 스트립 1번 칸 보조 문구. 빈 문자열이면 숨긴다. 문구는 반드시 현재 권한에서 **참**이어야 한다.</summary>
...
**`LoginCommand`는 제거한다** — 유일한 사용처였던 Ghost 버튼이 사라졌다. 남겨두면 "쓰이지 않는 진입점"이 되어 다음 사람이 배선을 오해한다.

`IsGuest`에 통지가 없어도 되는 근거는 F12다 — 홈 진입마다 `HomeViewModel`이 새로 생성되므로 로그인 후 복귀하면 새 인스턴스가 최신 상태를 읽는다.
