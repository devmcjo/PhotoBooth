...
| [web-step17-e2e-and-acceptance](./web-step17-e2e-and-acceptance.md) | **키오스크 웹 클라이언트 WBS Step 17**(★마일스톤 B — 마지막 Step) 상세 설계. Playwright 도입(`@playwright/test` 정확 핀 · chromium fake device + webkit `@camera` 제외)·**목 백엔드를 같은 오리진(`/__mock-api`)에 두는 근거**(Playwright가 CORS preflight를 가로채지 못해 교차 오리진 목은 성립하지 않는다 → `OPTIONS 204`는 실측 V20-1이 계속 소유)·**소스 백도어 없는 OAuth 모킹**(authorize 이동을 가로채 URL의 `state`·`redirect_uri`를 읽고 `/oauth2callback`을 재현 → `oauthCallbackRunner`가 실제로 돌아 AUTH-1·AUTH-4 무위반)·**E1~E24(E1b·E3b 포함) 26개 자동화 판정표**(자동 19·부분 6·재정의 1). **판정 4건**: ① **E3 재정의** — 게스트가 `Qr`에 도달할 수 없어(VF-11) "익명 업로드에 토큰 미부착"의 전제 상황이 제품에 없다 → E3-1/E3-2/E3b로 분해 ② **E6은 CDP 쿼터 override 조건부**(`getDirectory` 제거는 `skipped`가 되어 틀린 경로다) ③ **E13의 M15 축은 서버 등록**(로컬 저장은 비차단 경고) ④ E15·E19·E22의 잔여는 단위 테스트·V가 소유. 수락 체크리스트(10 §8) **3열 재구성**(자동/정적/사람)과 **V1~V25 통합 절차서 신설**(`docs/web-client/16` — 기기 3대 × 계정 5종의 세션 S1~S9, 정의의 진실원은 14 §10 유지) 포함 |
| [web-step16-account-usermgmt-diagnostics-pwa](./web-step16-account-usermgmt-diagnostics-pwa.md) | **키오스크 웹 클라이언트 WBS Step 16**(마지막 구현 Step) 상세 설계. `Account`(내 정보 + PIN 변경 + 관리자 도구 3항목)·`UserMgmt`(동급 허용 삭제 ↔ **동급 차단 PIN 재설정**의 비대칭을 도메인 함수 하나가 소유)·**진단 6섹션**(게이트 키는 "설정됨/미설정"만 · "구성됨 ≠ 도달 성공"을 2프로브로 구분)·**PWA/SW**(workbox 없이 두 번째 rollup 진입으로 `sw.js` 생성 · **기본 bypass 라우팅 분류기** · `skipWaiting`은 [지금 적용] 메시지 1경로 · 자산 목록을 sw.js에 인라인해야 업데이트가 감지된다)·**프레임 zip 내보내기/가져오기**(무압축 store zip 자체 구현 · 읽기는 deflate도 · Windows `Frame\` 접두 규약). **PIN 승인 단위를 화면 → 그룹으로 바꿔** `Account ↔ UserMgmt` 왕복의 재요구를 없앤 근거, **소스를 따라 문서를 고친 판정 4건**(`rolePolicy.ts` 부재 · `authMethodLabel` 문구 · 계정 팝오버 미구현 · 진단 6섹션), 앱에 **로그아웃 경로가 없던 결함**(F22) 정정 포함 |
