...
### 2. 프레임 편집 완성도 (item 2)
- [x] **2. 프레임 편집기 역할별 규칙 완성** — **완료**(서버 `f003565` `PUT /frames/{id}` + 클라 `7ec0e13`). 출처 판정·역할×출처 편집 권한(user 로컬만/manager 로컬+DB기본, `local:` 무검증 결함 제거)·기본프레임 편집 시 로컬/DB 팝업 + 이미지·slot diff·양 모드(레거시 SetAsync / HTTP PUT) 업데이트. Opus 리뷰 PASS.
