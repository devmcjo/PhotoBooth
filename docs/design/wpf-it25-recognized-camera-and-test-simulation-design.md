...
- ~~**위치**: 외부 카메라 하위 패널 안, 인식 콤보 캡션(W33) 바로 아래 우측 정렬 행.~~ → **개정(구현 시 팀리드 확정)**: **토글 행, 하위 패널 밖.** 이 목록이 답하는 질문("내 카메라가 지원되나?")은 토글을 켤지 결정하기 **전에** 필요하다 — 하위 패널 안에 두면 지원 여부를 몰라 아직 켜지 않은 사람이 목록에 도달할 수 없다. 이 프로젝트는 "상태 뒤에 UI를 숨겼다가" 두 번 지적받았다(게스트 섹션 Collapsed · 프린터 토글 강제 Disable). 배치는 `XamlResourceTests`의 위치 단정으로 못박는다.
- ~~**라벨**: `지원 카메라 목록`(W35) — `Button.Secondary`, 글리프 없음.~~ → ~~개정 1: 아이콘 전용 버튼(`Content="?"`, Width 44).~~ → **개정 2(확정, 2026-08-11)**: **텍스트 링크** `지원하는 제품 목록`(W35) — `TextBlock`(Text.Caption) 안의 `Hyperlink`.
  - **개정 경위**: 텍스트 버튼 → 44px 아이콘 버튼 → 텍스트 링크. 사용자 평가는 앞의 둘 모두 "과하게 크다"("버그 같아")였다. **원인은 크기가 아니라 컨트롤 종류였다** — 조작 비중이 낮은 열람 진입에 버튼(면적·테두리·배경)을 쓰면 그 행의 주역(토글)보다 시각 무게가 커진다. 같은 실수를 반복하지 않으려면 "이 컨트롤이 이 행에서 몇 번째로 중요한가"를 먼저 정하고 종류를 고른다.
  - **이 앱 최초의 `Hyperlink`다. 규약**: ① 색 `Brush.Accent.Text`(#D6376A — 팔레트가 흰 배경 **텍스트용**으로 따로 둔 값. `Brush.Accent` #FF4D79는 본문 텍스트로는 연해 대비가 떨어진다) ② 밑줄 상시(Hyperlink 기본 — 링크임을 알리는 유일한 시각 신호) ③ **hover 색 변경 없음**(더 연한 색은 대비를 떨어뜨린다. Hyperlink 기본 손가락 커서가 hover 피드백을 담당) ④ 신규 리소스 키 0 — 인라인 속성만.
  - **접근성**: 링크 글자가 그대로 접근성 이름이므로 `AutomationProperties`가 불요하다(아이콘 버튼 시절에는 필수였다). `Hyperlink`는 Tab 포커스·Enter 활성이 기본이다.
  - ⚠️ **hover 툴팁만으로 목록을 보여주는 안은 기각**(개정 1 단계에서 검토): ① 키오스크는 터치 화면일 수 있어 hover가 존재하지 않는다 ② 이 리포는 "hover 툴팁은 시인성이 낮아 폐지"를 이미 결정했다(`SettingsView.xaml` GuestGateNote 주석, it12 개정) ③ 제조사·제품명 표는 툴팁에 담기지 않는다. 목록은 항상 **클릭 오버레이**가 담당한다.
  - 연동: W33 캡션은 **화면에 실제로 보이는 글자**를 지목한다 → `지원 모델은 [지원하는 제품 목록]에서 확인하세요`. 없는 라벨(또는 사라진 글리프)을 안내하면 그 문구 자체가 거짓이 된다.
- **권한 게이트 없음**: 지원 모델 목록은 비밀이 아니다(라이선스 고지·QR 섹션이 게스트에게 보이는 것과 같은 논거). 열람은 편집이 아니므로 `CanEditExternalCamera`도 걸지 않는다.
