...
- ~~**라벨**: `지원 카메라 목록`(W35) — `Button.Secondary`, 글리프 없음.~~ → **개정(사용자 요구, 2026-08-11)**: 텍스트 버튼이 정보 조회 치고 과하게 컸다 → **아이콘 전용 버튼**(`Content="?"` · `ToolTip`/`AutomationProperties.Name`="지원 카메라 목록 보기" · `Button.Secondary` · `Width=44 Padding=0`). 이 섹션의 [↻ 카메라 다시 검색]과 같은 규격이라 신규 리소스 키가 없다.
  - ⚠️ **hover 툴팁만으로 목록을 보여주지 않는다**(대안으로 검토됐으나 기각): ① 키오스크는 터치 화면일 수 있어 hover가 존재하지 않는다 ② 이 리포는 "hover 툴팁은 시인성이 낮아 폐지"를 이미 결정했다(`SettingsView.xaml` GuestGateNote 주석, it12 개정) ③ 제조사·제품명 표는 툴팁에 담기지 않는다. 툴팁은 "이 버튼이 무엇인가"만 알리고 목록은 클릭 오버레이가 담당한다.
  - ⚠️ 라벨이 사라지므로 **툴팁·AutomationProperties가 이 버튼의 유일한 이름**이다 — 둘 중 하나라도 빠지면 정체 불명의 "?" 또는 이름 없는 버튼이 된다. 테스트가 둘을 함께 잠근다.
  - 연동: W33 캡션이 사라진 라벨을 지목하지 않도록 `지원 모델은 [?] 버튼에서 확인하세요`로 개정했다(없는 라벨을 안내하면 그 문구가 거짓이 된다).
