...
public abstract class HttpBackendClient
{
    ...
    {
        using var request = BuildRequest(HttpMethod.Get, relativeUrl, bearer);
    ...
    {
        using var request = BuildRequest(method, relativeUrl, bearer);
        if (body is not null)
    ...
    /// <summary>요청 조립: 상대 URL + API 키(항상) + Bearer(요청 시). 상대 URL은 BaseAddress에 결합된다.</summary>
    private HttpRequestMessage BuildRequest(HttpMethod method, string relativeUrl, bool bearer)
    {
