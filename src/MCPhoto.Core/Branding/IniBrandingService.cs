...
/// 읽기 전용(고객이 편집). 파일 부재/빈 값/손상 시 기본값 "MC포토"로 폴백.
...
public sealed class IniBrandingService : IBrandingService
{
    ...
    public const string DefaultAppName = "MC포토";
    ...
    public IniBrandingService(string? path = null, ILogger<IniBrandingService>? logger = null)
    {
        try
        {
            ...
            {
                // UTF-8 명시(BOM 유무 무관 안전). 손상 라인은 IniFile이 무시.
                ...
                {
                    AppName = name.Trim();
                    logger?.LogInformation("브랜딩 로드: AppName='{Name}' ({Path})", AppName, resolved);
                    return;
                }
            }
            logger?.LogInformation("브랜딩 설정 없음/빈 값 — 기본값 '{Default}' 사용", DefaultAppName);
        }
        catch (Exception ex)
        {
            // 어떤 실패에도 기본값으로 진행(앱 크래시 금지).
            logger?.LogWarning(ex, "브랜딩 로드 실패, 기본값 '{Default}' 사용", DefaultAppName);
        }
    }
